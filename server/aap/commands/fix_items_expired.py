...
class FixItemsExpiry(superdesk.Command):
    ...
    def run(self):
        ...
        if app.settings['PUBLISHED_CONTENT_EXPIRY_MINUTES']:
    ...
    def fix_items_expiry(self):
        ...
        for items in self.get_items(now):
            for item in items:
                current_expiry = item['expiry']
                time_diff = current_expiry - now

                if abs(time_diff.days) > (self.expiry_minutes / (24 * 60)):
    ...
    def get_items(self, now):
        ...
        no_of_pages = 0
        if count:
            no_of_pages = len(range(0, count, self.BATCH_SIZE))
        ...
        for page in range(0, no_of_pages):
            ...
            req.where = json.dumps(query)
            req.max_results = self.BATCH_SIZE
            cursor = get_resource_service(ARCHIVE).get_from_mongo(req=req, lookup=None)
            items = list(cursor)
            if len(items) > 0:
